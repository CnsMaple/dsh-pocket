...
// 默认 PIN 为 8 位数字；自定义 PIN 可使用 8–64 位英文字母（大小写）或数字。
...
/**
 * 用户自定义访问密码（issue #33）：公网/局域网各自设置固定的 8–64 位密码（英文字母大小写或数字）。
 * 自定义后公网开启时不再自动轮换（rotateAccessToken 见上）。
 * 非法输入（少于 8 位、超过 64 位或含字母数字以外字符）抛错，由 RPC 层转成错误响应。
 */
...
export function apply(ctx, config = {}, internals = {}) {
  ...
  const service = internals.service ?? createPocketService({
    ...
    home: internals.home ?? process.env.DSH_HOME ?? join(homedir(), '.dsh'),
    internals,
    ...
    // fork 功能：隧道就绪即把公网链接（纯 URL）推送到飞书机器人（settings.json 的 feishuWebhook）。
    onTunnelReady: (mode, url) => {
      let fresh;
      if (mode === 'named') {
        logger.info('dsh-pocket: named tunnel ready — PIN not rotated | 命名隧道就绪（公网密码不自动轮换）');
        fresh = getAccessToken();
      } else {
        fresh = rotateAccessToken();
        logger.info('dsh-pocket: public access PIN refreshed | 公网访问密码已更新（自定义密码不受影响）');
      }
      if (url) {
        const webhook = feishuWebhook();
        if (webhook) {
          void sendFeishuText(webhook, url, { secret: feishuSecret() })
            .then(() => logger.info('dsh-pocket: tunnel URL sent to Feishu | 公网链接已推送到飞书'))
            .catch((err) => logger.warn('dsh-pocket: Feishu push failed | 飞书推送失败: %s', err?.message ?? err));
        }
      }
      return fresh;
    },
  ...
  const disposeRpc = installPocketRpc(ctx, {
    ...
    setMobileRightbarEnabled: (on) => setMobileRightbarEnabled(on),
    getLanIpOverride: () => lanIpOverride(),
  ...
  void service.startProxy().then((proxy) => {
    ...
    // fork 功能：每次启动都自动开启公网隧道（就绪后经 onTunnelReady 推送飞书通知）。
    // 先走上游的标记恢复路径（issue #11/#121：日志与 marker 语义不变），
    // startTunnel 幂等，随后无条件再调一次作兜底 —— 已被恢复/复用则直接返回现有 URL。
    void (async () => {
      try { await service.restoreTunnelIfNeeded?.(); } catch { /* service 内部已记录 */ }
      try {
        const url = await service.startTunnel();
        logger.info('dsh-pocket: public tunnel auto-started on boot | 公网隧道已随启动自动开启: %s', url);
      } catch (err) {
        logger.warn('dsh-pocket: boot auto-start tunnel failed | 启动自动开启公网失败: %s', err?.message ?? err);
      }
    })();
  }).catch((err) => {
