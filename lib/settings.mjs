...
// fork 功能（启动自动开公网 → 推送飞书）的配置：直接写本 settings.json，无 UI。
//   "feishuWebhook": "https://open.feishu.cn/open-apis/bot/v2/hook/<token>"
//   "feishuSecret":  "机器人加签密钥（可选，留空不加签）"
/** 飞书自定义机器人 Webhook：空 = 未配置（不推送）。 */
export function feishuWebhook() {
  return String(readSettings().feishuWebhook ?? '').trim();
}

/** 飞书机器人加签密钥：空 = 不加签。 */
export function feishuSecret() {
  return String(readSettings().feishuSecret ?? '').trim();
}
