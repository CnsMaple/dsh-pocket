...
/** endpoint segment whitelist（与 dsh connection 一致）。 */
const ENDPOINT_SEGMENT_PATTERN = /^[A-Za-z0-9_$.-]+$/;
/** pocket RPC 体积上限：信封 + 小 JSON payload，无需沿用 connection 的大 body 默认值。 */
const POCKET_MAX_BODY_BYTES = 8 * 1024 * 1024;

function pocketEndpointFromPath(pathname) {
  if (!pathname.startsWith(`${POCKET_RPC_CHANNEL}/`)) return undefined;
  const endpoint = pathname.slice(POCKET_RPC_CHANNEL.length + 1);
  if (endpoint.split('/').some((segment) => segment === '' || segment === '.' || segment === '..' || !ENDPOINT_SEGMENT_PATTERN.test(segment))) return undefined;
  return endpoint;
}

/** 校验 client-request 信封，返回 zod-issues 风格数组（空数组 = 合法）。 */
function validatePocketEnvelope(body) {
  if (!body || typeof body !== 'object' || Array.isArray(body)) return [{ message: 'expected an object' }];
  const issues = [];
...
/**
 * 复刻 dsh connection.register 的 prefix 路由语义（rpcFetchHandler + bridge 的手写等价物）：
 * node (req, res) ⇄ rpcHandler(endpoint, payload, signal)。
 */
async function handlePocketRpcRequest(req, res, rpcHandler) {
  ...
  const endpoint = pocketEndpointFromPath(url.pathname);
  ...
  for await (const chunk of req) {
    ...
    if (received > POCKET_MAX_BODY_BYTES) {
  ...
  const issues = validatePocketEnvelope(body);
  if (issues.length > 0) {
    const rpcId = typeof body?.rpcId === 'string' && body.rpcId ? body.rpcId : 'invalid-request';
    return sendResponse(rpcId, { ok: false, error: { code: 'gateway/bad-request', message: 'invalid client-request message', details: { issues } } });
  }
  if (body.method !== endpoint) {
    return sendResponse(body.rpcId, {
      ok: false,
      error: { code: 'gateway/bad-request', message: `method ${JSON.stringify(body.method)} does not match endpoint ${JSON.stringify(endpoint)}`, details: { issues: [] } },
    });
  }
  try {
    return sendResponse(body.rpcId, await rpcHandler(endpoint, body.payload ?? {}, abort.signal));
  } catch (error) {
...
export function installPocketRpc(ctx, { service, log = console, desktop = false, runUpdate = null, restart = null, restartNotice = null, getToken = null, getLanToken = null, refreshLanToken = null, getLanAuthEnabled = null, setLanAuthEnabled = null, getLanEnabled = null, setLanEnabled = null, getLanIpOverride = null, setLanIpOverride = null, getPinCustom = null, setCustomPin = null, getTunnelConfig = null, setTunnelConfig = null, resetPocket = null }) {
  if (typeof ctx?.webServer?.register !== 'function' && !ctx?.connection?.rpc?.handle) {
  ...
  // dsh >= v0.1.3-alpha.2 起，`connection.rpc.handle` 在注册 prefix 路由时会从 connection
  // 插件自身的 fiber 解析 `webServer`（其 inject 里没有它），任何插件 fiber 调用必抛
  // `cannot get property "webServer" without inject`。改为直接挂到插件自己 inject 的
  // webServer 上，路由处理逐分支复刻 connection.register 的封装语义，线协议不变；
  // 旧版 dsh 没有 ctx.webServer 时退回 rpc.handle。
  if (typeof ctx?.webServer?.register === 'function') {
    return ctx.webServer.register({
      ...
      handler: async (req, res) => {
        ...
        await handlePocketRpcRequest(req, res, rpcHandler);
      },
    });
  }
  return ctx.connection.rpc.handle(POCKET_RPC_CHANNEL, rpcHandler, { authority: 'loopback' });
}
