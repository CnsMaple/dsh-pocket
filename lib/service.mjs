...
export function createPocketService({
  ...
  getTunnelConfig,
  /** 隧道就绪回调（lib/index.js 用它轮换公网密码并推送飞书；参数为 'quick'|'named' 与隧道 URL） */
  onTunnelReady,
    ...
    async startTunnel() {
      ...
      const p = (async () => {
        ...
        try {
          ...
          // 公网隧道就绪 → 回调（快速模式轮换访问密码；命名模式不轮换；附 URL 供飞书推送，见 lib/index.js）
          try { onTunnelReady?.(cfg?.mode === 'named' ? 'named' : 'quick', tunnel.url); } catch { /* 忽略 */ }
          return tunnel.url;
        } catch (err) {
